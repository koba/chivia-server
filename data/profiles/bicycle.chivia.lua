...
function setup()
  ...
  return {
    ...
    location_elevation_table = location_elevation_table()
  }
end
...
--- Chiviá customization ---

function location_elevation_table()
  local path = debug.getinfo(1).source:gsub('@', ''):gsub('bicycle.chivia.lua', '') .. '/data/bicycle.chivia_elevation.msp'
  local file = io.open(path, 'rb')
  local contents = file:read('*all')
  file:close()
  return msgpack.unpack(contents)
end

function process_segment(profile, segment)
  segment_source_key = segment.source.lat .. ',' .. segment.source.lon
  segment_target_key = segment.target.lat .. ',' .. segment.target.lon
  if profile.location_elevation_table[segment_source_key] ~= nil and profile.location_elevation_table[segment_target_key] ~= nil then
    segment_elevation = profile.location_elevation_table[segment_target_key] - profile.location_elevation_table[segment_source_key]
    segment.weight = segment.weight + segment_elevation
    io.write('[good] ' .. segment_elevation, '\n')
  else
    io.write('[bad] no elevation data for ' .. segment_source_key .. ' -> ' .. segment_target_key, '\n')
  end
end
